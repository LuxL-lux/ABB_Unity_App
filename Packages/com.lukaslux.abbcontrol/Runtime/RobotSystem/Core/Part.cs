...
namespace RobotSystem.Core
{
    ...
    public class Part : MonoBehaviour
    {
        ...
        [Header("Process Flow")]
        [SerializeField]
        private Station[] requiredStationSequence = new Station[0];

        [SerializeField]
        private bool enforceSequence = true;

        [SerializeField]
        private bool allowSkipStations = false;
        ...
        [SerializeField]
        private DateTime lastStationChangeTime;
        ...
        {
            if (!isInitialized || newStation == null)
                return false;
        ...
        {
            if (station == null || requiredStationSequence.Length == 0)
                return true;

            // Find target station in sequence
            int targetIndex = Array.FindIndex(
                requiredStationSequence,
                s => s != null && s.StationName == station.StationName
            );
            if (targetIndex == -1)
            ...
            int currentIndex = Array.FindIndex(
                requiredStationSequence,
                s => s != null && s.StationName == currentStation.StationName
            );
            if (currentIndex == -1)
                return false; // Current station not in sequence
        ...
        public bool MoveToStation(Station newStation)
        {
            ...
            {
                currentSequenceIndex = Array.FindIndex(
                    requiredStationSequence,
                    s => s != null && s.StationName == newStation.StationName
                );
            }
            else
            {
        ...
        {
            if (
                requiredStationSequence.Length == 0
                || currentSequenceIndex >= requiredStationSequence.Length - 1
            )
                return null;
        ...
        void OnDrawGizmosSelected()
        {
            ...
            if (requiredStationSequence.Length > 0)
            {
                ...
                {
                    if (
                        requiredStationSequence[i] != null
                        && requiredStationSequence[i + 1] != null
                    )
                    {
                        ...
                        Vector3 arrowHead1 =
                            toPos - direction * 0.2f + Vector3.Cross(direction, Vector3.up) * 0.1f;
                        Vector3 arrowHead2 =
                            toPos - direction * 0.2f - Vector3.Cross(direction, Vector3.up) * 0.1f;
