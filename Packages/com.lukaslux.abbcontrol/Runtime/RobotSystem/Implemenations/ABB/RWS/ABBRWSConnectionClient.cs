...
namespace RobotSystem.ABB.RWS
{
    public class ABBRWSConnectionClient : MonoBehaviour, IRobotConnector
    {
        ...
        [SerializeField]
        private bool enableMotionData = true;

        [SerializeField]
        private bool enableMetadata = false;

        [SerializeField]
        private int motionPollingIntervalMs = 50;

        [SerializeField]
        private string robName = "ROB_1";
        ...
        [Header("Data Parsing")]
        [SerializeField]
        private List<IRobotDataParser> dataParsers = new List<IRobotDataParser>();
        ...
        private void InitializeServices()
        {
            ...
            authService = new ABBAuthenticationService(
                robotIP,
                username,
                password,
                sharedHttpClient
        ...
        private IEnumerator ConnectSequence()
        {
            ...
            if (enableMetadata)
            {
                var subscriptionTask = subscriptionService.CreateSubscriptionAsync(
        ...
        private IEnumerator DisconnectSequence()
        {
            ...
            {
                var deleteTask = subscriptionService.DeleteSubscriptionAsync(
                    authService.SessionCookie
                );
        ...
        private void StartMotionDataService()
        {
            try
            {
                motionDataService = new ABBMotionDataService(
                    robotIP,
                    robName,
                    sharedHttpClient,
                    authService.SessionCookie
        ...
        private void OnJointDataReceivedInternal(float[] jointAngles)
        {
            if (jointAngles != null && jointAngles.Length >= 6)
            {
                ...
                // Update robot state
                robotState.UpdateJointAngles(
                    jointAngles,
                    motionDataService?.UpdateFrequency ?? 0.0
                );
        ...
        private void OnWebSocketDataReceived(RobotState state)
        {
            if (state != null)
            {
                ...
                {
                    robotState.UpdateProgramPointer(
                        state.currentModule,
        ...
        private void UpdateRobotStateFromParsed(RobotState parsedState)
        {
            ...
            if (
                !string.IsNullOrEmpty(parsedState.motorState)
                && parsedState.motorState != "unknown"
            )
            {
            ...
            if (
                !string.IsNullOrEmpty(parsedState.controllerState)
                && parsedState.controllerState != "unknown"
            )
            {
            ...
            if (
                !string.IsNullOrEmpty(parsedState.executionCycle)
                && parsedState.executionCycle != "unknown"
            )
            {
            ...
            {
                robotState.UpdateProgramPointer(
                    parsedState.currentModule,
                    parsedState.currentRoutine,
                    parsedState.currentLine,
                    parsedState.currentColumn
                );
            }
