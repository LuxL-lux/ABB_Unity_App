...
namespace RobotSystem.Safety
{
    ...
    public class CollisionDetectionMonitor : MonoBehaviour, IRobotSafetyMonitor
    {
        ...
        [SerializeField]
        private bool excludeProcessFlowLayer = true;
        public LayerMask CollisionLayers => GetFilteredCollisionLayers();

        [SerializeField]
        private bool useExistingCollidersOnly = false;

        [SerializeField]
        private float cooldownTime = 1.0f;

        [SerializeField]
        private List<string> criticalCollisionTags = new List<string> { "Machine", "Obstacles" };

        [Header("Robot Links")]
        [SerializeField]
        private List<Transform> robotLinks = new List<Transform>();

        [SerializeField]
        private bool autoFindRobotParts = true;
        ...
        private Dictionary<Transform, List<Collider>> robotColliders =
            new Dictionary<Transform, List<Collider>>();
        private Dictionary<string, DateTime> collisionCooldowns =
            new Dictionary<string, DateTime>();
        private List<RobotPartCollisionDetector> cachedDetectors =
            new List<RobotPartCollisionDetector>();
        ...
        public void Initialize()
        {
            ...
            {
                DebugLog(
                    $"[{MonitorName}] Initialization confirmed - {robotLinks.Count} robot links ready"
                );
            }
        }
        ...
        private void FindRobotParts()
        {
            ...
            System.Array.Sort(
                frames,
                (a, b) => GetHierarchyDepth(a.transform).CompareTo(GetHierarchyDepth(b.transform))
            );
        ...
        private MeshRenderer[] GetMeshesForFrame(Transform frameTransform)
        {
            ...
            foreach (var mesh in allMeshes)
            {
                ...
                {
                    if (childFrame.transform == frameTransform)
                        continue; // Skip self
        ...
        private void SetupCollisionDetection()
        {
            foreach (var robotPart in robotLinks)
            {
                ...
                if (useExistingCollidersOnly)
                {
                    ...
                    {
                        DebugLogWarning(
                            $"[{MonitorName}] No colliders found for frame {robotPart.name} - skipping collision detection"
                        );
                        continue;
                    }
                }
                ...
                foreach (var collider in colliders)
                {
                    ...
                    RobotPartCollisionDetector detector =
                        collider.gameObject.GetComponent<RobotPartCollisionDetector>();
                    if (detector == null)
                    {
        ...
        {
            if (!robotColliders.ContainsKey(part1) || !robotColliders.ContainsKey(part2))
                return;
        ...
        private bool IsExistingCollision(CollisionInfo newCollision)
        {
            ...
            {
                if (
                    existing.robotLink == newCollision.robotLink
        ...
        public void OnRobotPartCollision(string robotPartName, Collider hitCollider)
        {
            ...
            {
                DebugLogWarning(
                    $"[{MonitorName}] Null collider detected for part: {robotPartName}"
                );
                return;
            }
        ...
        private bool IsGrippedPart(Collider hitCollider)
        {
            ...
            if (gripper == null || !gripper.Gripped)
                return false;
