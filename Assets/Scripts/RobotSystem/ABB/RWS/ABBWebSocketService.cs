...
namespace RobotSystem.ABB.RWS
{
    public class ABBWebSocketService
    {
        ...
        public async Task<bool> SetupWebSocketAsync(string websocketUrl, string sessionCookie)
        {
            ...
            {
                Debug.LogError("[ABB WebSocket] Cannot setup WebSocket: No WebSocket URL available");
                return false;
            }
            ...
            try
            {
                ...
                websocket.OnMessage += (bytes) =>
                {
                    ...
                    {
                        Debug.LogError($"[ABB WebSocket] Error processing WebSocket message: {ex.Message}");
                    }
                };
                ...
                {
                    Debug.LogError($"[ABB WebSocket] WebSocket setup failed. State: {websocket.State}");
                    OnConnectionStateChanged?.Invoke(false);
                    return false;
                }
            }
