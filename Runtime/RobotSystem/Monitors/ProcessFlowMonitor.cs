...
namespace RobotSystem.Safety
{
    ...
    public class ProcessFlowMonitor : MonoBehaviour, IRobotSafetyMonitor
    {
        ...
        [SerializeField]
        private bool monitorAllParts = true;

        [SerializeField]
        private List<Part> specificParts = new List<Part>();

        [SerializeField]
        private bool autoDiscoverParts = true;

        [SerializeField]
        private bool autoDiscoverStations = true;

        [Header("Violation Settings")]
        [SerializeField]
        private bool treatSkippedStationsAsWarning = true;

        [SerializeField]
        private bool treatWrongSequenceAsCritical = true;

        [SerializeField]
        private float violationCooldownTime = 1.0f;
        ...
        private Dictionary<string, Station> partLastKnownStations =
            new Dictionary<string, Station>(); // Track last station for each part
        ...
        public void Initialize()
        {
            ...
            {
                DebugLogWarning(
                    $"[{MonitorName}] Initialize called but component not properly pre-initialized in Awake"
                );
            }
            else
            {
        ...
        {
            if (!IsActive || part == null || station == null)
                return;

            // Check if this part should be monitored
            if (!ShouldMonitorPart(part))
                return;
        ...
        private void HandlePartEntered(
            Part part,
            Station station,
            bool wasGripped,
            bool isGripped,
            ...
            if (!wasGripped && !isGripped)
            {
                ...
                if (lastStation != null && lastStation != station)
                {
                    ...
                    if (part.IsValidNextStation(station))
                    {
                        ...
                        DebugLog(
                            $"[{MonitorName}] Part '{part.PartName}' placed at {station.StationName} ({part.GetCompletionPercentage():F1}% complete)"
                        );
                    }
                    else
                    {
                ...
                else if (part.CurrentStation == null || part.CurrentStation == station)
                {
                    ...
                    if (part.CurrentStation == null)
                    {
                        part.MoveToStation(station);
                        DebugLog(
                            $"[{MonitorName}] Part '{part.PartName}' initialized at {station.StationName}"
                        );
                    }
                }
            }
        ...
        private void HandlePartExited(Part part, Station station, bool wasGripped, bool isGripped)
        {
            if (!wasGripped && isGripped)
            {
                ...
                DebugLog(
                    $"[{MonitorName}] Part '{part.PartName}' picked up from {station.StationName}"
                );
            }
        ...
        private void OnInvalidStationTransition(
            Part part,
            Station fromStation,
            Station attemptedStation
        )
        {
            if (!IsActive)
                return;
        ...
        private void HandleInvalidStationEntry(Part part, Station station)
        {
            ...
            {
                if (
                    (DateTime.Now - lastViolationTime[violationKey]).TotalSeconds
                    < violationCooldownTime
                )
                {
                    return;
                }
        ...
        private void HandleInvalidStationTransition(
            Part part,
            Station fromStation,
            Station attemptedStation
        )
        {
            ...
            {
                ProcessViolationType.WrongSequence => treatWrongSequenceAsCritical
                    ? SafetyEventType.Critical
                    : SafetyEventType.Warning,
                ProcessViolationType.SkippedStation => treatSkippedStationsAsWarning
                    ? SafetyEventType.Warning
            ...
            string description =
                $"Process flow violation: Part '{part.PartName}' attempted invalid transition from {violationData.fromStationName} to {violationData.attemptedStationName}. "
        ...
        private ProcessViolationType DetermineViolationType(
            Part part,
            Station fromStation,
            Station attemptedStation
        )
        {
            ...
            int fromIndex =
                fromStation != null
                    ? Array.FindIndex(
                        sequence,
                        s => s != null && s.StationName == fromStation.StationName
                    )
                    : -1;
            int attemptedIndex = Array.FindIndex(
                sequence,
                s => s != null && s.StationName == attemptedStation.StationName
            );
            ...
            if (fromIndex == -1)
            {
                ...
                return attemptedIndex == 0
                    ? ProcessViolationType.ValidTransition
                    : ProcessViolationType.WrongSequence;
            }
